...
const ReviewPage = () => {
  ...
  const { firstname, lastname, email, tel, address, city, state, postalcode, object, profilePicture, language1, language2, dateofbirth, workExperiences } = formData || {};
  ...
  return (
    <div className="container-fluid">
      <div className="row">
        <div className="col-md-6 col-sm-12" id="alisha">
          <div className="d-flex justify-content-around bg-primary p-4 flex-wrap">
            <div className="mt-2 p-2">
              {profilePicture && <img src={URL.createObjectURL(profilePicture)} alt="Profile" style={{ height: "80px", width: "80px", border: "1px solid grey", borderRadius: "50%" }} />}
            </div>
            <div className="mt-2 p-2">
              <h5 style={{ color: "white", whiteSpace: 'nowrap' }}>
                {firstname} {lastname}
              </h5>
              <p style={{ color: "white", whiteSpace: 'nowrap' }}>as a {workExperiences?.[0]?.jobTitle}</p>
            </div>
            <div id='hello' className="mt-2 p-2">
              ...
              <p style={{ whiteSpace: '' }}>
                <span className="material-icons text-white" >home</span> {address} {city} {state} {postalcode}
              </p>
            </div>
          </div>
